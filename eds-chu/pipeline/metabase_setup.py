...
CARTES_PILOTAGE = [
    ...
    ("Taux de réadmission à 30 jours par service",
     "SELECT service_label, taux_readmission_pct, nb_readmissions FROM v_taux_readmission_par_service ORDER BY taux_readmission_pct DESC",
     "bar", {"graph.dimensions": ["service_label"], "graph.metrics": ["taux_readmission_pct"]}, HALF, 8),
    ("Alertes monitoring par jour (%)",
     "SELECT * FROM v_alertes_monitoring_par_jour ORDER BY jour",
     "line", {"graph.dimensions": ["jour"], "graph.metrics": ["pct_alertes"]}, HALF, 8),
    ("Alertes monitoring par service",
     "SELECT service_label, pct_alertes, nb_alertes FROM v_alertes_par_service ORDER BY pct_alertes DESC",
     "bar", {"graph.dimensions": ["service_label"], "graph.metrics": ["pct_alertes"]}, HALF, 8),
